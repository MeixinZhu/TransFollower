...
DATASET = 'SH' # ['SH', 'NGSIM']
exp_name = f'{DATASET}_{MODEL}_autoshift'
...
if MODEL == 'transfollower':    
    model = Transfollower(enc_in= 11).to(device)
elif MODEL == 'lstm':
...
if MODEL == 'nn':
    train_loader, val_loader = get_data(data_name = DATASET, label_len = SEQ_LEN)
else:
    train_loader, val_loader = get_data(data_name= DATASET)
...
def val(data_loder):
    ...
    with torch.no_grad():
        for i, item in enumerate(data_loder):
            enc_inp = item['his'][:,:, [0,1,3]].float()

            enc_inp_shift1 = torch.cat([torch.zeros(enc_inp.shape[0], 5, 2).float(), enc_inp[:,:-5,1:]], axis = 1) # shift spacing and lv speed
            enc_inp_shift2 = torch.cat([torch.zeros(enc_inp.shape[0], 5*2, 2).float(), enc_inp[:,:-5*2,1:]], axis = 1) 
            enc_inp_shift3 = torch.cat([torch.zeros(enc_inp.shape[0], 5*3, 2).float(), enc_inp[:,:-5*3,1:]], axis = 1) 
            enc_inp_shift4 = torch.cat([torch.zeros(enc_inp.shape[0], 5*4, 2).float(), enc_inp[:,:-5*4,1:]], axis = 1)
            enc_inp = torch.cat([enc_inp, enc_inp_shift1, enc_inp_shift2, enc_inp_shift3, enc_inp_shift4], axis = -1).to(device)
