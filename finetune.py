...
MODEL = 'transfollower' 
DATASET = 'SH' # ['SH', 'NGSIM']
exp_name = f'{DATASET}_{MODEL}_fintune'
...
with open(f'checkpoints/SH_transfollower_pretrain_model.pt', 'rb') as f:
...
def val(data_loder):
    ...
    with torch.no_grad():
        for i, item in enumerate(data_loder):
            enc_inp = item['his'][:,:,[0,1,3]].float().to(device)
            ...
            # encoder - decoder
            out = model(enc_inp, dec_inp, task = 'reg')
